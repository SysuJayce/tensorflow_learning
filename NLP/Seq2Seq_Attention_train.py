...
class NMTModel(object):
    ...
    def forward(self, src_input, src_size, trg_input, trg_label, trg_size):
        ...
        # #####################注意力机制==============================
        # 构造编码器时，使用bidirectional_dynamic_rnn构造双向循环神经网络
        # 双向循环神经网络的顶层输出enc_outputs是一个包含两个张量的tuple，每个
        # 张量的维度都是[batch_size, max_time, HIDDEN_SIZE]，代表两个LSTM
        # 在每一步的输出
        with tf.variable_scope('encoder'):
            enc_outputs, enc_state = tf.nn.bidirectional_dynamic_rnn(
                self.enc_cell_fw, self.enc_cell_bw, src_emb,
        ...
        # #####################注意力机制==============================
        with tf.variable_scope('decoder'):
            # 选择注意力权重的计算模型。BahdanauAttention是使用一个隐藏层的前馈
            # 神经网络。memory_sequence_length是一个维度为[batch_size]的张量，
            # 代表batch中每个句子的长度， Attention需要根据这个信息把填充位置的
            # 注意力权重设置为0
            attention_mechanism = tf.contrib.seq2seq.BahdanauAttention(
                HIDDEN_SIZE, enc_outputs, memory_sequence_length=src_size)

            # 将解码器的循环神经网络self.dec_cell和注意力一起封装成更高层的
            # 循环神经网络
            ...
            # 这里没有指定init_state，也就是没有使用编码器的输出来初始化输入，
            # 而完全依赖注意力作为信息来源
...
def main():
    ...
    cost_op, train_op = train_model.forward(src, src_size, trg_input, trg_label,
                                            trg_size)
